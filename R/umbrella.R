...
#' @param JAMOVI a logical vector only used the change the format of error messages (not of any use for R users)
#'
#' @details
...
umbrella = function (x, method.var = "REML", mult.level = FALSE, r = 0.5, pre_post_cor = 0.8, method.esb = "TESSPSST", true_effect = "UWLS", tau2 = NULL, max_asymmetry = 100, seed = NA, verbose = TRUE, JAMOVI = FALSE) {
  ...
  for (factor in unique(x$factor)) {
    ...
    x_i_mlm <- .identify_multilevel(x_i, mult.level, verbose, r, JAMOVI)
